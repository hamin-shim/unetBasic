...
class UNETPP(nn.Module):
    def __init__(self, model_info, init_weights=True):
        ...
        nest_conv0_x = []
        for i in range(4):
            nest_conv0_x.append(double_conv(
                (model_info['encoder'][0][1] + model_info['encoder'][0][0] * (i + 1)), model_info['encoder'][0][0]))

        self.nest_conv0 = nn.ModuleList([*nest_conv0_x])

        nest_conv1_x = []
        for i in range(3):
            nest_conv1_x.append(double_conv(
                (model_info['encoder'][1][1] + model_info['encoder'][1][0] * (i + 1)), model_info['encoder'][1][0]))

        self.nest_conv1 = nn.ModuleList([*nest_conv1_x])

        nest_conv2_x = []
        for i in range(2):
            nest_conv2_x.append(double_conv(
                (model_info['encoder'][2][1] + model_info['encoder'][2][0] * (i + 1)), model_info['encoder'][2][0]))

        self.nest_conv2 = nn.ModuleList([*nest_conv2_x])

        nest_conv3_x = []
        nest_conv3_x.append(double_conv(
            (model_info['encoder'][3][1] + model_info['encoder'][3][0]), model_info['encoder'][3][0]))

        self.nest_conv3 = nn.ModuleList([*nest_conv3_x])

        self.last = nn.Conv3d(
            model_info['encoder'][0][0], model_info['out_channel'], 1)
    ...
    def forward(self, x):
        ...
        x = self.last(x0_4)
        # print(x.shape)
        # mask = F.interpolate(x, size=(155, 240, 240),
